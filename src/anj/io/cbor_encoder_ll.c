...
size_t anj_cbor_ll_encode_uint(void *buffer, uint64_t value) {
    return encode_type_and_number(buffer, CBOR_MAJOR_TYPE_UINT, value);
}

size_t anj_cbor_ll_encode_int(void *buffer, int64_t value) {
    if (value >= 0) {
        return anj_cbor_ll_encode_uint(buffer, (uint64_t) value);
    }
...
size_t anj_cbor_ll_encode_bool(void *buffer, bool value) {
    return write_cbor_header(buffer, CBOR_MAJOR_TYPE_FLOAT_OR_SIMPLE_VALUE,
                             value ? CBOR_VALUE_BOOL_TRUE
                                   : CBOR_VALUE_BOOL_FALSE);
}

size_t anj_cbor_ll_encode_float(void *buffer, float value) {
    uint32_t portable = _anj_htonf(value);
...
size_t anj_cbor_ll_encode_double(void *buffer, double value) {
    if (((float) value) == value) {
        return anj_cbor_ll_encode_float(buffer, (float) value);
    }

    uint64_t portable = _anj_htond(value);
    size_t bytes_written =
            write_cbor_header(buffer, CBOR_MAJOR_TYPE_FLOAT_OR_SIMPLE_VALUE,
...
size_t anj_cbor_ll_indefinite_string_begin(void *buffer) {
    return write_cbor_header(buffer, CBOR_MAJOR_TYPE_TEXT_STRING,
                             CBOR_EXT_LENGTH_INDEFINITE);
}

size_t anj_cbor_ll_indefinite_bytes_begin(void *buffer) {
    return write_cbor_header(buffer, CBOR_MAJOR_TYPE_BYTE_STRING,
                             CBOR_EXT_LENGTH_INDEFINITE);
}
