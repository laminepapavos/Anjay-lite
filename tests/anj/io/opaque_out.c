...
ANJ_UNIT_TEST(opaque_out, entry_already_added) {
    opaque_test_env_t env;
...
ANJ_UNIT_TEST(opaque_out, format_type_not_set) {
...
ANJ_UNIT_TEST(opaque_out, no_remaining_bytes_bytes) {
    opaque_test_env_t env;
    opaque_test_setup(&env);
...
ANJ_UNIT_TEST(opaque_out, unsupported_format_type) {
...
ANJ_UNIT_TEST(opaque_out, get_external_bytes_handler_error) {
    opaque_test_env_t env;
    opaque_test_setup(&env);
...
ANJ_UNIT_TEST(opaque_out, get_external_bytes_handler_null) {
    opaque_test_env_t env;
    opaque_test_setup(&env);
...
ANJ_UNIT_TEST(opaque_out, open_external_bytes_handler_error) {
    opaque_test_env_t env;
    opaque_test_setup(&env);
    ...
    input.value.external_data.open_external_data = external_data_open_failure;
    input.value.external_data.close_external_data = external_data_close;
    env.entry = input;
    opened = false;
    closed = false;

    ANJ_UNIT_ASSERT_EQUAL(_anj_io_out_ctx_new_entry(&env.ctx, &env.entry), -1);

    ANJ_UNIT_ASSERT_FALSE(opened);
    ANJ_UNIT_ASSERT_FALSE(closed);
}

ANJ_UNIT_TEST(opaque_out, open_external_bytes_handler_null) {
    opaque_test_env_t env;
    opaque_test_setup(&env);
...
ANJ_UNIT_TEST(opaque_out, close_external_bytes_handler_null) {
    opaque_test_env_t env;
    opaque_test_setup(&env);
...
#    define TEST_BYTES(Name, DataInput)                                       \
        ANJ_UNIT_TEST(opaque_out, bytes_##Name) {                             \
            anj_io_out_entry_t input = { 0 };                                 \
            input.type = ANJ_DATA_TYPE_BYTES;                                 \
...
#        define TEST_BYTES_EXT(Name, DataInput)                   \
            ANJ_UNIT_TEST(opaque_out, bytes_ext_##Name) {         \
                anj_io_out_entry_t input = { 0 };                 \
                input.type = ANJ_DATA_TYPE_EXTERNAL_BYTES;        \
...
#    define TEST_BYTES_EMPTY_CHUNK_LENGTH(Name, DataInput, DataExpected) \
        ANJ_UNIT_TEST(opaque_out, bytes_empty_ch_len_##Name) {           \
            anj_io_out_entry_t input = { 0 };                            \
            input.type = ANJ_DATA_TYPE_BYTES;                            \
            input.value.bytes_or_string.chunk_length = 0;                \
            input.value.bytes_or_string.data = DataInput;                \
...
ANJ_UNIT_TEST(opaque_out, bytes_offset) {
    char DataInput[] =
...
#    define TEST_BYTES_COMPLEX(Name, DataInput, buffer_length)                \
        ANJ_UNIT_TEST(opaque_out, bytes_complex_##Name) {                     \
            anj_io_out_entry_t input = { 0 };                                 \
            input.type = ANJ_DATA_TYPE_BYTES;                                 \
            input.value.bytes_or_string.chunk_length = sizeof(DataInput) - 1; \
            input.value.bytes_or_string.data = DataInput;                     \
...
#        define TEST_EXT_BYTES_COMPLEX(Name, DataInput, buffer_length) \
            ANJ_UNIT_TEST(opaque_out, bytes_ext_complex_##Name) {      \
                anj_io_out_entry_t input = { 0 };                      \
                input.type = ANJ_DATA_TYPE_EXTERNAL_BYTES;             \
                input.value.external_data.get_external_data =          \
                        external_data_handler;                         \
